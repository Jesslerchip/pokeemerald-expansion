...
{
    AddItemMenuActionTextPrinters(windowId, 7, 8, 1, 0, 16, gBagMenu->contextMenuNumItems, sACTIONs, gBagMenu->contextMenuItemsPtr);
...
{
    PrintMenuActionGrid(windowId, 7, 8, 1, 56, columns, rows, sACTIONs, gBagMenu->contextMenuItemsPtr);
...
static void Task_ItemContext_SingleRow(u8 taskId)
{
    if (MenuHelpers_CallLinkSomething() != TRUE)
    {
        ...
        case MENU_B_PRESSED:
            PlaySE(SE_SELECT);
            sACTIONs[ACTION_CANCEL].func.void_u8(taskId);
            break;
        default:
            PlaySE(SE_SELECT);
            sACTIONs[gBagMenu->contextMenuItemsPtr[selection]].func.void_u8(taskId);
            break;
        }
    }
}
...
static void Task_ItemContext_MultipleRows(u8 taskId)
{
    if (MenuHelpers_CallLinkSomething() != TRUE)
    {
        ...
        {
            PlaySE(SE_SELECT);
            sACTIONs[gBagMenu->contextMenuItemsPtr[cursorPos]].func.void_u8(taskId);
        }
